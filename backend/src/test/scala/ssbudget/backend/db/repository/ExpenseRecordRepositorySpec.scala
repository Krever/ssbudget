...
class ExpenseRecordRepositorySpec extends RepositorySpec {
  ...
  private def setupPeriodAndExpense(
    ...
    val expense = ExpenseDefinition(ExpenseDefId("exp-1"), "Rent", ExpenseType.Planned, EstimateMode.Fixed, Some(200000L), true)
  ...
  "findByPeriod returns all records for that period" in {
    ...
    val expense = ExpenseDefinition(ExpenseDefId("exp-1"), "Rent", ExpenseType.Planned, EstimateMode.Fixed, Some(100L), true)
